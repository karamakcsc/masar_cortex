...
def setup(expense_entry, method):
    ...
    if expense_entry.vat:
        for v in expense_entry.vat:
            total += float(v.amount)        
            count += 1
            
            if not v.project and expense_entry.default_project:
                v.project = expense_entry.default_project
            
            if not v.cost_center and expense_entry.default_cost_center:
                v.cost_center = expense_entry.default_cost_center

            vat_items.append(v)
        expense_entry.vat = vat_items
    ...
    if expense_entry.status == "Approved":         
        ...
        if expense_entry.vat:
            for v in expense_entry.vat:            
                ...
                accounts.append({  
                    'debit_in_account_currency': float(v.amount),
                    'user_remark': str(v.description),
                    'account': v.vat_account,
                    'project': v.project,
        ...
        if expense_entry.bank_account and (not expense_entry.payment_reference or not expense_entry.clearance_date):
            frappe.throw(
                title="Enter Payment Reference",
                msg="Payment Reference and Date are Required for bank payments."
            )
            
        if (expense_entry.mode_of_payment and expense_entry.mode_of_payment != "Cash" and (not 
        ...
        if expense_entry.mode_of_payment == "Cash":
            expense_entry.clearance_date = ""
            expense_entry.payment_reference = ""

        if expense_entry.mode_of_payment:
        ...
        elif expense_entry.bank_account:
            pay_account = frappe.db.get_value("Bank Account", {"name": expense_entry.bank_account}, "account")
        
        if not pay_account:
            frappe.throw(
                title=_("Error"),
                msg=_("No account is linked to the selected payment method.")
            )
        ...
        if expense_entry.accumulated_balance == 0:    
            ...
            if expense_entry.vat:
                for v in expense_entry.vat:
                    accounts.append({  
                        'credit_in_account_currency': float(v.amount),
                        'user_remark': str(v.description),
                        'account': pay_account,
                        'cost_center': expense_entry.default_cost_center
                    })
        ...
        je = frappe.get_doc({
            ...
            'mode_of_payment': expense_entry.mode_of_payment if expense_entry.mode_of_payment else None,
