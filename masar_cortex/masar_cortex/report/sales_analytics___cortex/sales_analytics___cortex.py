...
class Analytics:
	...
	def get_period(self, posting_date):
		...
		# elif self.filters.range == "Accounting Period":
		# 	year = get_fiscal_year(posting_date, company=self.filters.company)
		# 	frappe.msgprint(str(year[0]))
		# 	period = str(year[0])
		else:
			year = get_fiscal_year(posting_date, company=self.filters.company)
	...
	def get_period_date_ranges(self):
		...
		increment = {"Monthly": 1, "Quarterly": 3, "Half-Yearly": 6, "Yearly": 12, "Accounting Period": 1}.get(self.filters.range, 1)

		if self.filters.range in ["Monthly", "Quarterly"]:
			from_date = from_date.replace(day=1)
		elif self.filters.range in ["Yearly", "Accounting Period"]:
