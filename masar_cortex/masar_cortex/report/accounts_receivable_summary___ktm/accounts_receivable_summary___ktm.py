...
class AccountsReceivableSummary(ReceivablePayableReport):
	...
	def get_data(self, args):
		...
		# Filter advance amounts by cost center if specified
		if self.filters.get("cost_center"):
			party_advance_amount = self.filter_advance_by_cost_center(party_advance_amount)
	...
	def filter_advance_by_cost_center(self, party_advance_amount):
		"""Filter advance payment amounts by cost center"""
		if not party_advance_amount:
			return {}
		
		cost_center = self.filters.get("cost_center")
		filtered_advances = {}
		
		# Get payment entries with cost center filter
		payment_entries = frappe.db.sql("""
			...
			WHERE pe.docstatus = 1
				...
				AND pe.cost_center = %(cost_center)s
				AND pe.party_type IN %(party_type)s
			GROUP BY pe.party
		""", {
		...
		for entry in payment_entries:
			filtered_advances[entry.party] = flt(entry.advance_amount)
...
def get_gl_balance(report_date, company, cost_center=None):
	filters = {
